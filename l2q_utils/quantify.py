...
def test_quantifiers(quantifiers, transformed_data):
    ...
    for m, df_test in enumerate(generate_tests_by_month(transformed_data)):
        logging.info(f"Testing quantifiers on iteration m={m}.")
...
def test_quantifiers_bootstraped(quantifiers, data, n_bootstraps: int = 30, fraction: int = 1):
    tests = pd.DataFrame()

    # Month 6, 7, 8
    logging.info(f"Start Testing with n={n_bootstraps} bootstraps fraction={fraction}.")

    for m in range(6, 9):
        logging.info(f"Testing m={m} for {n_bootstraps} bootstraps.")
...
def do_n_tests_with_subsampling_on_qlist(
    ...
    logging.info(f"Start Test {N} StratifiedSuffleSplit itrerations.")
    ...
    for i, test_index in sss.split(X_test, y_test):
        ...
        logging.info(f"{i} itreration.")
        result = _quantify(quantifiers, X_test.loc(test_index), y_test.loc(test_index))
        results = pd.concat([results, pd.DataFrame.from_records(result)])
