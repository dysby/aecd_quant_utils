...
def select_threshold(target, predicted, max_fpr = 0.01):
    ...
    """
    df = pd.DataFrame(zip(target, predicted), columns=['target', 'scores']).sort_values(by='scores', ascending=False)
    ...
    if len(thresholds) == 0:
        return df.scores.head(1).item() #never classifify as 1, threfore will never result in false positive.
...
def recall_at_1perc_pfr_threshold(preds, train_data):
    higher_is_better = True
    
    labels = train_data.get_label()
    #scores = 1. / (1. + np.exp(-preds))
    p_scores = expit(preds)

    trh = select_threshold_1perc_fpr(labels, p_scores)
    recall_at_1perc_fpr = recall_score(y_eval, np.where(p_scores < trh, 0, 1))

    
    return 'recall_at_1perc_fpr', recall_at_1perc_fpr, higher_is_better

def recall_at_5perc_pfr_threshold(preds, train_data):
    higher_is_better = True
    
    labels = train_data.get_label()
    p_scores = 1. / (1. + np.exp(-preds))

    trh = select_threshold_5perc_fpr(labels, p_scores)
    recall_at_5perc_fpr = recall_score(y_eval, np.where(p_scores < trh, 0, 1))

    return 'recall_at_5perc_fpr', recall_at_5perc_fpr, higher_is_better
...
def objective(trial, train_df, eval_df):
    ...
    param = {
        ...
        'learning_rate' : trial.suggest_float("learning_rate", 1e-5, 0.2), #loguniform(1e-5, 0.2),
        'num_iterations' : trial.suggest_int("num_iterations", 10, 1000), #randint(10,1000),       # randomly sample numbers from 10 to 500 estimators
        'max_bin': trial.suggest_int("max_bin", 25, 255), #randint(25, 255),
        'num_leaves': trial.suggest_int('num_leaves', 20, 500), #randint(20, 500), 
        'min_child_samples': trial.suggest_int('min_child_samples',0, 500), #randint(0, 500), 
        'min_child_weight': trial.suggest_float('min_child_weight', 1e-5, 1e4), # loguniform(1e-5, 1e4),
    ...
    gbm = lgb.train(
        param, train_df, valid_sets=[eval_df], verbose_eval=False#, callbacks=[pruning_callback]
    )
